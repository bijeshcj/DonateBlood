...
public class OperationsModel extends BaseModel {
    ...
    protected List<Operation> mOperations = new ArrayList<Operation>();

    static OperationsModel mOperationModel = null;
    ...
    public boolean addOperation(Operation aOpr) {
        for (Operation opr : mOperations) {
            if (opr.id == aOpr.id) {
                if (opr.getState() == Operation.ERROR || opr.getState() == Operation.FINISHED) {
    ...
    public void removeAllOperations() {
        // check if operatation list is already empty.
        if(mOperations == null || mOperations.isEmpty()){
            return;
        }
